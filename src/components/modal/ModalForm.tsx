...
const ModalForm: React.FC<ModalFormProps> = ({ onClose, isOpen }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const timer = setTimeout(() => setIsVisible(false), 300); // 300ms animatsiya davomiyligi bilan moslashadi
  ...
  return (
    isVisible && (
      <div className={`fixed inset-0 bg-black bg-opacity-25 backdrop-blur-sm flex justify-center items-center ${isOpen ? 'animate-fadeIn' : 'animate-fadeOut'}`}>
        <div className="bg-gray-100 flex items-center justify-center rounded-md">
          <div className="bg-gray-200 p-6 rounded-lg shadow-lg max-w-xl w-full relative">
